...
class ReportController extends BasicController
{
	...
	function init()
	{
		...
		parent::setControllerHelp('Report');
	...
	/**
	 * Exibe formulário para buscar informações do relatório
	 */
	...
	{		
		...
		$this->cleanViewModules();
		$this->view->module1 = ReportBusiness::getModuleRelationship();
		$form->setModule1(null);
		$this->render(DEFAULT_INDEX_ACTION);
	}
	...
	function reportAction()
	{
		...
		$this->setModuleToPreView($form);
		
		$errorMessages=null;
		ReportValidator::validateData($form, $errorMessages, true);
		if(count($errorMessages) > 0)
		{
			$this->view->errorMessages = $errorMessages;
			$this->render('index');
		...
		$this->view->report = ReportBusiness::load(array($mod1, $mod2, $mod3, $mod4), $this->assembleFilter(), $this->assembleOrder(), $limit=true);
		$this->view->limit = Zend_Registry::get(CONFIG)->report->limit;
		
		self::verifyDateFilterValue($form);
	...
	/**
	 * verifica os formatos de datas
	 */
	public function verifyDateFilterValue($form)
	{
		if (ereg ("([0-9]{4})-([0-9]{2})-([0-9]{2})", $form->getFilterValue1()))
			$form->setFilterValue1(BasicForm::dateFormatForm($form->getFilterValue1())); 
		else 
			$form->getFilterValue1();
		
		if (ereg ("([0-9]{4})-([0-9]{2})-([0-9]{2})", $form->getFilterValue2()))
			$form->setFilterValue2(BasicForm::dateFormatForm($form->getFilterValue2())); 
		else 
			$form->getFilterValue2();
			
		if (ereg ("([0-9]{4})-([0-9]{2})-([0-9]{2})", $form->getFilterValue3()))
			$form->setFilterValue3(BasicForm::dateFormatForm($form->getFilterValue3())); 
		else 
			$form->getFilterValue3();
			
		if (ereg ("([0-9]{4})-([0-9]{2})-([0-9]{2})", $form->getFilterValue4()))
			$form->setFilterValue4(BasicForm::dateFormatForm($form->getFilterValue4())); 
		else 
			$form->getFilterValue4();
	}
...
//					$this->view->fields3 = (ReportBusiness::loadFields($mod3));
					...
					if($selected[0] == CLS_CLASSMODEL)
						$selected[] = CLS_CLASSASSISTANCE;
						
					if($selected[0] == CLS_CLASSASSISTANCE)
						$selected[] = CLS_CLASSMODEL;
...
//						$selected = array_merge((array)$selected, (array)$mod4);			
	...
	public function setModuleToPreView($form)
	{
		if(empty($form))
			return ;
...
//						$selected = array_merge((array)$selected, (array)$mod4);			
	...
	/**
	 * Monta um array que com os valores que compõe a cláusula OrderBy 
	 */
	...
	/**
	 * Retorna o valor na forma de array
	 */
