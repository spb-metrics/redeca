 ...
 */
define('F_ID', 			'id');
define('F_PROFILE_NAME','profileName');
...
class ProfileForm extends BasicForm
{
	...
	public function assembleRequest(&$_request)
	{
		...
		$this->id = $_request->getParam(ProfileForm::id());
		if($_request->isPost())
		{
			$filter				= BasicForm::getFilterStripTags();
			$this->id 			= $_request->getPost(ProfileForm::id());
			$this->profileName 	= trim($filter->filter($_request->getPost(ProfileForm::profileName())));
			$this->status 		= trim($filter->filter($_request->getPost(ProfileForm::status())));
		}
	}	
