...
class SocialProgramBuilderQuery extends BuilderQueryAbstract
{
	...
	protected static $tables = array(
		CLS_SOCIALPROGRAM => array(
				self::_PREFIX => self::SOCIALPROGRAM_PREFIX,
				self::_MAIN_TABLE_KEY => TRUE,
				self::_REFERENCE_FIELDS_KEY => array(
					CLS_SOCIALPROGRAMTYPE => array(
										self::SOCIALPROGRAM_PREFIX => SPG_ID_SOCIAL_PROGRAM,
										self::SOCIALPROGRAMTYPE_PREFIX => SCP_ID_SOCIAL_PROGRAM)
										)
				),
		CLS_SOCIALPROGRAMTYPE => array(
				self::_PREFIX => self::SOCIALPROGRAMTYPE_PREFIX,
				self::_REFERENCE_FIELDS_KEY => array(
					CLS_SOCIALPROGRAMORIGIN => array(
										self::SOCIALPROGRAMTYPE_PREFIX => SCP_ID_ORIGIN,
										self::SOCIALPROGRAMORIGIN_PREFIX => SPO_ID_ORIGIN)
										)
				),
		CLS_SOCIALPROGRAMORIGIN => array(self::_PREFIX => self::SOCIALPROGRAMORIGIN_PREFIX),
	);
	...
	public static function getInstance()
	{
        ...
        {
        	self::$instance = new SocialProgramBuilderQuery();
    	}
	...
	public function build()
	{
		...
		$sql[self::_WHERE_KEY] = self::SOCIALPROGRAM_PREFIX.'.'.SPG_STATUS.' is null OR '.self::SOCIALPROGRAM_PREFIX.'.'.SPG_STATUS.' = ""';
		return $sql;
	}
}
